                            ...
                            <div class="card card-transparent shadow-none d-flex justify-content-center mb-0 auth-card">
                                <div class="card-body">
                                    <a href="{{ route('dashboard.index') }}"
                                        ...
                                        @if (isset($settings))
                                            <img class="logo" src="{{ showFile($settings->logo_header) }}"
                                                alt="">
                                        @else
                                    ...
                                    <form id="loginForm" method="POST" action="{{ route('admin.login') }}"
                                        data-toggle="validator" class="mt-4">
                                        {{ csrf_field() }}
                                        <div class="row">
                                            <div class="col-lg-12">
                                                <div class="form-group">
                                                    <label for="email"
                                                        class="form-label">{{ __('auth.email') }}</label>
                                                    <input id="email" type="email" name="email"
                                                        class="form-control"
                                                        placeholder="{{ __('auth.placeholderEmail') }}" required
                                                        autofocus>
                                                </div>
                                            </div>
                                            <div class="col-lg-12">
                                                <div class="form-group">
                                                    <label for="password"
                                                        class="form-label">{{ __('auth.password') }}</label>
                                            ...
                                            <div class="col-lg-6">
                                                <div class="form-check mb-3">
                                                    ...
                                                    <label class="form-check-label"
                                                        for="customCheck1">{{ __('auth.rememberme') }}</label>
                                                </div>
                                            </div>
                                            <div class="col-lg-6">
                                            </div>
                                        ...
                                        <div class="d-flex justify-content-center">
                                            <button type="submit"
                                                class="btn btn-primary">{{ __('auth.sign in') }}</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                ...
                $.ajax({
                    ...
                    error: function(errorObj, errorText, errorThrown) {
                        ...
                        if (errorObj.status === 500) {
                            errorToster('{{ __('auth.an error occurred') }}',
                                "{{ __('auth.code error') }}");
